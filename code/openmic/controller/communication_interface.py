...
logger = logging.getLogger('openmic.controller.communication_interface')
...
class CommunicationInterface:
	...
	def _initialize_driver(self):
		...
		if self.driver_name == constants.DRIVER_ZEROMQ :
			self.driver_request = ZMQ.controller_forward_requests
			self.driver_response = ZMQ.controller_get_responses
		elif self.driver_name == constants.DRIVER_RABBITMQ :
			self.driver_request = RMQ.controller_forward_requests
			self.driver_response = RMQ.controller_get_responses
		else:
